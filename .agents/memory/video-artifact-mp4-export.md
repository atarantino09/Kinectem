...
3. **Mux the composite audio.** Audio is NOT captured by recordVideo. The artifact's `public/audio/composite_audio.mp3` is already lockstep with the visual timeline starting at t=0, so trim the webm at `perfStart` and overlay the audio from 0: `ffmpeg -ss <perfStart> -i raw.webm -i composite_audio.mp3 -t <TOTAL_MS/1000> -map 0:v:0 -map 1:a:0 -c:v libx264 -pix_fmt yuv420p -crf 22 -preset slow -movflags +faststart -c:a aac -b:a 160k out.mp4`.
