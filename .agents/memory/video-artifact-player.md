...
- **Non-iframed (export/recording): `ExportPlayer`** — autoplay once, no loop, fires `window.startRecording()` on mount and `window.stopRecording()` on end (the capture pipeline injects those globals). App-preview screenshots hit this path, so they show autoplay, not the interactive controls.
