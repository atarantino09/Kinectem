...
- **Stripe org billing — card-on-file now, first charge Oct 1**: Signup stays card-free; orgs pick a plan via `OrgSubscribePage.tsx` (`PUT /organizations/:orgId/subscription`). Adding a card is **deferred, opt-in**: "Add a card on file" → `POST /organizations/:orgId/billing/checkout-session` (Stripe Checkout, `mode: subscription`, `trial_end` = Oct 1 2026 so **nothing is charged until then**, `metadata.orgId`). Return URL hits the page with `?billing=success&session_id=...`, which calls `POST /organizations/:orgId/billing/reconcile` to persist `stripeCustomerId`/`stripeSubscriptionId` (then auto-charges yearly on Oct 1). Both billing endpoints are `requireAuth` + `canManageOrganization` + `billingCheckoutLimiter`. Stripe key comes from the **Replit `stripe` connector** (`settings.secret`, fallback `STRIPE_SECRET_KEY`) via `src/lib/stripeClient.ts` — **the connector has NO webhook secret**; `src/routes/stripe-webhook.ts` is optional and only mounts when `STRIPE_WEBHOOK_SECRET` env is set (reconcile-on-return is the primary path, webhook is belt-and-suspenders). Plan prices/coupons are auto-provisioned in Stripe by lookup_key (`kinectem_<plan>_yearly`, `kinectem_promo_<code>`). `hasCardOnFile` in `serializeSubscription` = `stripeSubscriptionId != null`. `stripe` is a dep of **api-server** (not root). The Sept-15 reminder copy lives in `scripts/src/send-billing-reminders.ts` (self-contained — leaf scripts can't import api-server email helpers).
